...
function Statbox({ label, value, sub, subHighlight, index = 0 }) {
  return (
    ...
    initial={{ opacity: 0, y: 20 }}
      ...
      whileHover={{ y: -4 }}
    className='relative overflow-hidden bg-[#16162A]/90 backdrop-blur-2xl border border-violet-500/20 rounded-xl p-3 md:p-4 flex flex-col gap-1 shadow-[0_8px_32px_rgba(0,0,0,0.4)] hover:border-violet-500/40 hover:shadow-[0_12px_40px_rgba(124,58,237,0.15)] transition-all'>
        ...
        {sub && (
            <div className='relative flex items-center gap-1 mt-0.5 flex-wrap'>
                {subHighlight && (
                    <div className='text-[9px] font-semibold bg-violet-500/15 text-violet-300 px-1 py-0.5 rounded'>{subHighlight}</div>
                )}
